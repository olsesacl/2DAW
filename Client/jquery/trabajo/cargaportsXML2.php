...
$ports["2"]["nombre_zona"] = "Pirineus";
$ports["2"]["port"]["1"]["nom"] = "Tourmalet";
...
$ports["2"]["port"]["2"]["distancia"] = "23";
$ports["2"]["port"]["2"]["altura"] = "2404";
$ports["2"]["port"]["2"]["color_misatge"] = "red";
$ports["2"]["port"]["2"]["misatge"] = "Extremar les precaucions amb les boires repentines, visibilitat molt defectuosa";
$ports["2"]["port"]["2"]["datos"] = "El puerto de Envalira es el puerto de montaña con carretera más alto de los Pirineos, con 2.409 metros de altura sobre el nivel del mar, situado en Andorra entre las localidades de Soldeu y Pas de la Casa y el único acceso desde Andorra hacia Francia.";

$ports["2"]["port"]["3"]["nom"] = "Aubisque";
$ports["2"]["port"]["3"]["distancia"] = "22";
$ports["2"]["port"]["3"]["altura"] = "1709";
$ports["2"]["port"]["3"]["color_misatge"] = "red";
$ports["2"]["port"]["3"]["misatge"] = "Precaució amb els osos salvatges";
$ports["2"]["port"]["3"]["datos"] = "Se trata de un paso habitual del Tour de Francia, catalogado como una subida de categoría especial. El primer paso se realizó en el Tour de Francia de 1910, y desde entonces se ha subido con una frecuencia mayor a una vez cada dos años.";

$ports["3"]["nombre_zona"] = "Alps";
$ports["3"]["port"]["1"]["nom"] = "Alpe d'Huez";
$ports["3"]["port"]["1"]["distancia"] = "14";
$ports["3"]["port"]["1"]["altura"] = "1845";
$ports["3"]["port"]["1"]["color_misatge"] = "red";
$ports["3"]["port"]["1"]["misatge"] = "Extremar les precaucions amb les boires repentines, visibilitat molt defectuosa";
$ports["3"]["port"]["1"]["datos"] = "Alpe d'Huez es una de los más famosos finales de etapa de la ronda francesa. Apareció en el trazado de la Grande Boucle por primera vez en 1952, con la victoria del italiano Fausto Coppi y, aunque no se volvió a subir hasta el Tour de 1976 desde entonces ha estado presente en el recorrido de la carrera casi todos los años. Desde la edición del 95 parece que la frecuencia por la que han optado los organizadores para incluirla en el recorrido, es la de un año sí, un año no, salvo 2003 y 2004, incluida en ambas ocasiones, cambiando su secuencia de años impares a los pares.<br />Esquema de desnivel de Alpe d'Huez.<br />Su trascendencia en el desarrollo de la carrera se fue incrementando con cada nueva edición, hasta el punto de que, durante algunos años, era famosa la máxima de que \"quien sale de amarillo del Alpe d'Huez, gana el Tour de Francia\".<br /> Sin embargo, tras los Tours del 87 y del 89, cuando ni Pedro Delgado ni Laurent Fignon pudieron hacerse con la victoria final en la general después de haber salido líderes de Alpe d'Huez, se dijo que además de salir líder del Alpe d'Huez, había que ganar una contrarreloj.";

$ports["3"]["port"]["2"]["nom"] = "Stelvio";
...
function llistar_ports($list_ports,$llitat_ports, &$elementos_xml){
  foreach($list_ports["port"] as $codi=>$dato) {
    $elementos_xml[] = "<puerto>";
    $elementos_xml[] = "<codi>" . $codi . "</codi>";
    $elementos_xml[] = "<codi_cordillera>" . $llitat_ports . "</codi_cordillera>";
    $elementos_xml[] = "<nom>" . $dato["nom"] . "</nom>";
    $elementos_xml[] = "</puerto>";
  }
}


if(!empty($_POST["seleccion"]) && !empty($_POST["puerto"])){
 ...
 // foreach($ports_select["port"] as $allports) {
    $elementos_xml[] = "<puerto>";
    foreach($ports[$seleccion]["port"][$puerto] as $tag => $dato) {
